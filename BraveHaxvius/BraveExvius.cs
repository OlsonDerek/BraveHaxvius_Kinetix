...
namespace BraveHaxvius
{
    ...
    {
        public String AppVersion { get; set; } = "1100";
        public String RscVersion { get; set; } = "0";
        public String MstVersion { get; set; } = "1957";
        ...
        //public String OperatingSystem { get; set; } = "android9";
        public String Locale { get; set; } = "US";
        public String BuildVersion { get { return Locale == "JP" ? "ver.2.9.3" : "ver.3.5.0"; } }
        ...
        {
            var gachaList = GetUserInfo[GameObject.GachaMst] ?? Network.SendPacket(Request.GachaEntry)[GameObject.GachaMst][0];
        ...
        {
            News.Clear();
            var newsList = (GetUserInfo[GameObject.NoticeMstNew] ?? Network.SendPacket(Request.RoutineHomeUpdate)[GameObject.NoticeMstNew]).Select(n => new News
            {
        ...
        public JObject DoMission(Mission mission, Boolean useFriend = false, String itemHax = null,
            ...
            Boolean collectUnits = false, Boolean exploreTreasure = false, String lbIncrease = null, UInt16 SleepTime = 0, bool harvestLoot = true)
        {
            ...
            var min = (int)(TotalDamage * 1.05f);
            var max = (int)(TotalDamage * 1.45f);
            max = max > 0 ? max : Int32.MaxValue;
            TotalDamage = new Random().Next(min, max);
            ...
            if (MissionInfo[GameObject.HarvestDetailInfo] != null)
                ItemsFound = harvestLoot ? String.Join(",", MissionInfo[GameObject.HarvestDetailInfo]?.Select(h => h[Variable.HarvestItem].ToString().Substring(0, h[Variable.HarvestItem].ToString().LastIndexOf(":")))) : "";
            ...
            if (collectLoot)
            {
                ...
                {
                    Logger.Out("failed to parse Loot! " + e.Message);
                }
            }
            ...
            if (exploreTreasure)
            {
                ...
                {
                    Logger.Out("failed to parse Exploration Treasures! " + e.Message);
                }
            }
        ...
        public JObject SetParty(Int32 partyId)
        {
        ...
        public void ClearArena()
        {
            ...
            while (orbs > 0)
            {
                ...
                foreach (var partyUnit in partyUnits)
                {
                    ...
                    playerDmg.Add(unit.UnitId + ":" + unit.UniqueUnitId + ":" + unit.UnitHp.Split(new char[1] { '-' })[0] + ":0");
                    totalDmg += dmg;
                }
        ...
        public void ClearRaid(Mission doRaidMission, int raidPartyId)
        {
            ...
            {
                SetParty(raidPartyId);
            }

            while (RaidOrbs > 0)
            {
                var missionResult = DoMission(doRaidMission, collectLoot: true, SleepTime: 3000);
