...
namespace BraveHaxvius.Data
{
    ...
    {";
        ...
        static void DumpTranslation(JToken testMst, JToken msts, String translation, String mst, String className, Dictionary<String, String> members = null)
        {
            /*if (testMst != null)
            {
                try
                {
                    var translations = DecodeFile(testMst[0].First(x => x[Variable.KeyName].ToString() == translation));
                    var json = DecodeFile(testMst[0].First(x => x[Variable.KeyName].ToString() == mst));
                    DumpTranslation(translations, json, translation, mst, className, members);
                }
                catch(Exception e)
                {
                    Console.WriteLine("Can't find new translations.");
                    Console.WriteLine(e);
                }
            }*/
        ...
        static void DumpTranslation(List<String> translations, List<String> json, String translation, String mst, String className, Dictionary<String, String> members = null)
        {
            ...
            if (className == "Mission")
            {
                classVars += "        public String NameId { get { return Name + \" | \" + MissionId; } }" + Environment.NewLine;
            }
            ...
            foreach (var line in translations)
            {
                ...
                if (name == "Special \"Vault of Hidden Treasures\" unlocked!")
                {
                    name = "Special 'Vault of Hidden Treasures' unlocked!";
                }
                ...
                foreach (var member in members)
                {
                    ...
                    if (value != null)
                    {
                        if (value == "Special \"Vault of Hidden Treasures\" unlocked!")
                        {
                            value = "Special 'Vault of Hidden Treasures' unlocked!";
                        }
                        if (value == "Tiana\"s Prism")
                        {
                            value = "Tiana's Prism";
                        }
        ...
        static void Main(string[] args)
        {
            ...
            //DumpHardcoded("Mission", responseJson);
            var bot = new BraveExvius
            {
                MstVersion = "0",
                AppVersion = "1101"
            ...
            var init = bot.Network.SendPacket(Request.Initialize);
            var testMsts = init[GameObject.VersionInfo].First(x => x[Variable.KeyName].ToString() == "F_LOCALIZED_TEXT_MST");
            var newMsts = init[GameObject.VersionInfo];
            {
                var F_LOCALIZED_TEXT_MST = DecodeFile(newMsts.First(m => m[Variable.KeyName].ToString() == "F_LOCALIZED_TEXT_MST"));
                F_LOCALIZED_TEXT_MST.ForEach(l => {
...
";
            ...
            DumpTranslation(testMsts, newMsts, "F_TEXT_ITEM_EQUIP_NAME", "F_EQUIP_ITEM_MST", "Equipment");
            DumpTranslation(testMsts, newMsts, "F_TEXT_UNITS_NAME", "F_UNIT_MST", "Unit", new Dictionary<String, String>  {
                { "UniqueUnitId", "" },
                { "Level", "" },
                { "Tmr", "" },
                { "Equip", "" } });
            DumpTranslation(testMsts, newMsts, "F_TEXT_MISSION", "F_MISSION_MST", "Mission");
            DumpTranslation(testMsts, newMsts, "F_TEXT_IMPORTANT_ITEM_NAME", "F_IMPORTANT_ITEM_MST", "ImportantItem");
            DumpTranslation(testMsts, newMsts, "F_TEXT_ITEM_NAME", "F_ITEM_MST", "Item");
            DumpTranslation(testMsts, newMsts, "F_TEXT_RECIPE_BOOK_NAME", "F_RECIPE_BOOK_MST", "Recipe");
            DumpTranslation(testMsts, newMsts, "F_TEXT_MATERIA_NAME", "F_MATERIA_MST", "Materia");
            DumpTranslation(testMsts, newMsts, "F_TEXT_CHALLENGE_NAME", "F_CHALLENGE_MST", "Challenge");
            {
